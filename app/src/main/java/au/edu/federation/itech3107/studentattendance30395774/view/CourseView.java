package au.edu.federation.itech3107.studentattendance30395774.view;
...
public class CourseView extends FrameLayout {
    ...
    List<CourseBaseBean> mCourseList = new ArrayList<>();

    private CourseBaseBean mAddTagCourse;
    ...
    private void initCourseItemView() {
        ...
        for (CourseBaseBean course : mCourseList) {
            realAddCourseItemView(course);
        }
    }

    /** Add data after interface initialization */
    public void addCourse(CourseBaseBean course) {
        if (course == null) {
            return;
        }
    ...
    private void realAddCourseItemView(CourseBaseBean course) {
    ...
    private void updateItemStatus(CourseBaseBean course) {
        /*Update the active status of the course*/
        course.setActiveStatus(course.shouldShow(mCurrentIndex));
    }

    private void setItemViewBackground(CourseBaseBean course, TextView tv) {
    ...
    private void addTagCourseView(int x, int y) {
        ...
        if (mAddTagCourse == null)
            mAddTagCourse = new CourseBaseBean();
     ...
     */
    @SuppressLint("ClickableViewAccessibility")
    private View createItemView(final CourseBaseBean course) {
        final BackgroundView bgLayout = new BackgroundView(getContext());
        //TextView
    ...
    private void itemEvent(final CourseBaseBean course, final ViewGroup viewGroup, TextView textView) {
        final List<CourseBaseBean> courses = new ArrayList<>();
        courses.add(course);

        /*Finds items that overlap within the range of items clicked*/
        for (CourseBaseBean findCourse : mCourseList) {
    ...
    public abstract static class OnItemClickListener {
        public abstract void onClick(List<CourseBaseBean> course, View itemView);

        public abstract void onLongClick(List<CourseBaseBean> course, View itemView);

        public abstract void onAdd(CourseBaseBean course, View addView);
    }
